...
$(document).ready(function() {
	$('#eduQuestionTreeTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/questionbank/questiontree/eduQuestionTree/data2",
	  ...
	  $('#eduQuestionTreeTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#addToHomework,#removeHomework').prop('disabled', ! $('#eduQuestionTreeTable').bootstrapTable('getSelections').length);
        });
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location.href="/jeeplus/static/common/template/questions.docx";
				  },
...
//				}
//			};
//		$('#eduQuestionTreeTable').bootstrapTable('refresh',opt);
	 });
	 ...
	 $("#back").click(function(){//返回前一页
		 window.location.href = "${ctx}/homework/eduhomework/eduHomework/list";
	  });
  ...
  function refresh(){
   ...
   function doAddToHomework(){
	   var ids = getIdSelections();
	   if(!ids){
		   alert("请选择题目！");
		   return;
	   }
	   jp.post("${ctx}/homework/eduhomeworkquestion/eduHomeworkQuestion/saveAll?questions=" + ids,
           {
               "homework":homework
           },
           function (data) {
   ...
   function doRemoveHomework(){
	   var ids = getIdSelections();
	   if(!ids){
		   alert("请选择题目！");
		   return;
	   }
	   jp.post("${ctx}/homework/eduhomeworkquestion/eduHomeworkQuestion/removeAll?questions=" + ids,
           {
               "homework":homework
           },
           function (data) {
