						...
						if(completeStudents.length > 0){
							...
							for(i = 0 ; i <= student;i++){
								$(".completeStudents").append(`<li>
																	...
																	<span class="rightspan fontColorGray">提交时间：`+completeStudents[i].updateDate+` </span>
																</li>`);
							}
						}
						//学习最多的教学资源
						...
						if(stduyMostResources.length > 0){
							...
							for(i = 0 ; i <= resouce;i++){
								$(".stduyMostResources").append(`<li>
																		<span class="leftspan fontColorBlack"><span class="name" title="`+(i+1)+`.`+ stduyMostResources[i].resourcename +`">`+(i+1)+`.`+ stduyMostResources[i].resourcename +`</span><i>`+ stduyMostResources[i].browse +`次</i></span>
																		<p class="fontColorGray">`+ checkStr(stduyMostResources[i].subject) +`</p>
																		<span class="rightspan fontColorGray">上传时间：`+ stduyMostResources[i].createDate +`  </span>
																</li>`);
							}
						}
						//采用次数最多的题目
	...
	<!--理想尺寸1180，要适配更大的屏幕-->
		<div class="systemindex__top">
			<div class="systemindex__tleft">
				<div class="systemindex__tltop bcolor-cyan">
					<!--<img src=""/>图片暂缺-->
				</div>
				<div class="systemindex__tlbottom">
					<div class="systemindex__tlbleft bcolor-blue">
						<h4>发布教学资源总数</h4>
