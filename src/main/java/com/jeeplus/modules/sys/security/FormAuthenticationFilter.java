...
public class FormAuthenticationFilter extends org.apache.shiro.web.filter.authc.FormAuthenticationFilter {
	...
	protected void issueSuccessRedirect(ServletRequest request,
		...
		if (p != null && !p.isMobileLogin()){
			...
			String loginType = httpRequest.getHeader("loginType");
			if(!validLoginType(roleList,loginType)){
				j.setSuccess(false);
				j.setErrorCode("-2");
				j.setMsg("登录权限错误!");
			...
			if(loginType.equals("3")){
				User student = userMapper.getStudentByParent(p.getId());
				if(null != student){
			...
			j.put("photo", Global.getConfig("fileUrl") + p.getPhoto());
			j.put("JSESSIONID", p.getSessionid());
			PrintJSON.write((HttpServletResponse)response, j.getJsonStr());
		}
	}

	private boolean validLoginType(List<Role> roleList,String loginType){
		boolean flag = false;
		if(StringUtils.isEmpty(loginType) || null == roleList || roleList.size() == 0){
			return flag;
		}
		for(Role role : roleList){
			...
			if("1".equals(loginType)){
				...
				if("parent".equals(role.getEnname())){
					flag = true;
					break;
				}
			}
