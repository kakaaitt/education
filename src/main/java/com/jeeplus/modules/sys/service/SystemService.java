...
import com.jeeplus.modules.sys.mapper.AppVersionMapper;
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public AppVersion getLastVersion(AppVersion appVersion){
		return appVersionMapper.getLastVersion(appVersion);
	}
	
	public EduLoginLog isLoginToday(String user){
		return userMapper.isLoginToday(user);
	}
	
	@Transactional(readOnly = false)
	public void insertLoginLog(EduLoginLog eduLoginLog){
		userMapper.insertLoginLog(eduLoginLog);
	}
	
	public User getStudentByParent(String parentid){
		return userMapper.getStudentByParent(parentid);
	}
