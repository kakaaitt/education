...
//	@RequiresPermissions("sys:user:index")
	...
	public String index(User user, Model model) {
		...
		Role role = new Role();
		if("admin".equals(loginUser.getRole().getEnname())){
...
//	@RequiresPermissions("sys:user:index")
	...
	public Map<String, Object> data(User user,Integer type, HttpServletRequest request, HttpServletResponse response, Model model) {
		//type-----1、学生,2、家长,3、老师
		if(null != type){
			String roleName = "";
			if(type == 1){
		...
		user.setSchoolId(request.getParameter("amp;schoolId"));
		user.setClassesId(request.getParameter("amp;classesId"));
...
//				currentUser.setPhoto(user.getPhoto());
	...
	public String condition(HttpServletRequest request, Model model) {
		...
		for(Cookie cookie : cookies){
			if(cookie.getName().equals("jeeplus.session.id")){
				System.out.println("JSESSIONID ： " + cookie.getValue());
				model.addAttribute("JSESSIONID", cookie.getValue());
			}
		}
		return "modules/sys/user/userCondition";
	}
}
